...
type CityCollection = {
  slug: 'nlpremium' | 'nlheritage' | 'nlcanals';
  label: string;
  subtitle: string;
  accent: string;
};
...
const COLLECTIONS: CityCollection[] = [
  {
    ...
    subtitle: 'Amsterdam, Rotterdam, The Hague, Utrecht, Eindhoven and more.',
    accent: 'from-[#AE1C28] via-[#FFFFFF] to-[#21468B]',
  },
  {
    slug: 'nlheritage',
    label: 'Heritage Cities',
    subtitle: 'Delft, Leiden, Alkmaar, Zwolle, Amersfoort and more.',
    accent: 'from-[#2D6A4F] via-[#95D5B2] to-[#F7F7FF]',
  },
  {
    slug: 'nlcanals',
    label: 'Canals & Coast',
    subtitle: 'Giethoorn, Volendam, Marken, Texel, Scheveningen and more.',
    accent: 'from-[#0D3B66] via-[#F4D35E] to-[#F7F7FF]',
  },
];

const PHOTO_SLICES = [
  { title: 'Amsterdam', query: 'Amsterdam Netherlands canals city', position: 'center top' },
  { title: 'Rotterdam', query: 'Rotterdam Netherlands skyline city', position: 'center 35%' },
  { title: 'Delft', query: 'Delft Netherlands canals city', position: 'center 50%' },
  { title: 'Giethoorn', query: 'Giethoorn Netherlands canals village', position: 'center bottom' },
] as const;

const buildPhotoUrl = (query: string) => `https://source.unsplash.com/featured/1200x900/?${encodeURIComponent(query)}`;

export default function NetherlandsCityPage() {
    ...
    <main className="min-h-screen bg-[#070707] px-6 py-20 text-white md:px-16">
      <section className="mx-auto max-w-6xl">
        <div className="rounded-3xl border border-white/15 bg-[#0D0D0D] p-8 shadow-[0_30px_80px_rgba(0,0,0,0.45)]">
          <p className="text-xs font-bold uppercase tracking-[0.3em] text-[#21468B]">Dope&Cute Studio</p>
          <h1 className="mt-3 text-4xl font-black md:text-5xl">Netherlands City Collection</h1>
          <p className="mt-3 max-w-3xl text-sm text-white/70 md:text-base">
            Casquettes inspirees des villes neerlandaises. Broderie du nom de ville,
