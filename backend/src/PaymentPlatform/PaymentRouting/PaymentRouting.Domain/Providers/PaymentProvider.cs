...
public sealed class PaymentProvider
{
        ...
        int priority)
    {
        ...
        if (baseCostScore < 0)
            throw new ArgumentOutOfRangeException(nameof(baseCostScore));
    ...
    public static PaymentProvider Restore(
        string providerId,
        string displayName,
        PaymentRail rail,
        IReadOnlyCollection<string> countries,
        ...
        DateTime updatedAtUtc)
    {
        if (updatedAtUtc.Kind != DateTimeKind.Utc)
            throw new ArgumentException("Provider updated timestamp must be UTC.", nameof(updatedAtUtc));

        var provider = new PaymentProvider(
            providerId, displayName, rail, countries, currencies, baseCostScore, priority);

        provider.SetHealth(status, successRate, averageLatencyMs, updatedAtUtc);
        return provider;
    }
    ...
    public void UpdateHealth(
        ...
        double successRate,
        double averageLatencyMs) =>
        SetHealth(status, successRate, averageLatencyMs, DateTime.UtcNow);

    public bool Supports(string countryCode, string currencyCode)
    {
        return Countries.Contains(countryCode.ToUpperInvariant(), StringComparer.OrdinalIgnoreCase)
               && Currencies.Contains(currencyCode.ToUpperInvariant(), StringComparer.OrdinalIgnoreCase);
    }
        ...
        DateTime updatedAtUtc)
    {
        ...
        if (averageLatencyMs < 0)
            throw new ArgumentOutOfRangeException(nameof(averageLatencyMs));
