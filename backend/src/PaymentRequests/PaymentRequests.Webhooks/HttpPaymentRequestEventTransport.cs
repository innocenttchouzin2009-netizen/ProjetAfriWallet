...
public sealed record PaymentRequestWebhookHttpDeliveryOptions(Uri Endpoint)
{
    public void Validate()
    {
        ...
        {
            throw new InvalidOperationException("Payment request webhook endpoint must be an absolute HTTP or HTTPS URI.");
        }
    }
}
    ...
    : IPaymentRequestEventTransport
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        request.Headers.TryAddWithoutValidation(PaymentRequestWebhookHeaderNames.EventId, headers.EventId.ToString("D"));
        request.Headers.TryAddWithoutValidation(PaymentRequestWebhookHeaderNames.Timestamp, headers.TimestampUnixSeconds.ToString(System.Globalization.CultureInfo.InvariantCulture));
