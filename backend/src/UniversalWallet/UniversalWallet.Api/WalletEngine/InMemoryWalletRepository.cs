...
public sealed class InMemoryWalletRepository : IWalletRepository
{
    ...
    private readonly ICurrencyRegistryRepository? _currencyRegistry;

    public InMemoryWalletRepository(ICurrencyRegistryRepository? currencyRegistry = null)
    {
    ...
    public IReadOnlyList<LedgerEntry> GetLedger(Guid walletId)
    {
        ...
        public IReadOnlyList<string> SupportedCurrencies() => SupportedCurrencyCodes();

    private IReadOnlyList<string> SupportedCurrencyCodes()
    {
		var currencies = _currencyRegistry?.List().Where(currency => currency.Status == CurrencyStatus.Active).Select(currency => currency.Code).ToList();
		return currencies ?? CurrencyList.ToList();
    }
