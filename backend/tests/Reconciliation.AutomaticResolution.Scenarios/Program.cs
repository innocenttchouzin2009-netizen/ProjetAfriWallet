...
var alreadyApproved = exact with
{
    ReviewId = Guid.NewGuid(),
...
Assert(
    terminalDecision.Disposition == AutomaticResolutionDisposition.ManualReview,
    "Non-pending items must never be auto-decided again.");
Assert(
    terminalDecision.Command is null,
    "Non-pending items must not emit a new command.");

var suspiciousExact = exact with
{
    ReviewId = Guid.NewGuid(),
    AmountDifferenceMinor = 1
};

Assert(
    policy.Evaluate(suspiciousExact).Disposition == AutomaticResolutionDisposition.ManualReview,
...
AssertThrows<ArgumentException>(
    ...
    "Non-UTC automatic resolution timestamp must be rejected.");

AssertThrows<ArgumentException>(
    () => orchestrator.Plan(exact, queuedAt.AddSeconds(-1)),
    "Automatic resolution timestamp before queue time must be rejected.");

Console.WriteLine("AFW-BE-RECONCILIATION-AUTO-RESOLUTION-1 scenarios: PASS");
