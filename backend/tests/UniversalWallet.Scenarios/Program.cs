...
var balanceProjectionRepository = new InMemoryBalanceProjectionRepository();
var balanceSnapshotRepository = new InMemoryBalanceSnapshotRepository();
var projectionVersionRepository = new InMemoryProjectionVersionRepository();
var currencyReader = new WalletCurrencyReader(walletRepository);
var balanceService = new BalanceProjectionService(ledgerRepository, currencyReader, balanceProjectionRepository, balanceSnapshotRepository, projectionVersionRepository);
var walletReadModelService = new WalletReadModelService(walletRepository, ledgerRepository, balanceService);
...
Run("balance projection from ledger only", () =>
{
	var sourceWallet = SeedWallet("AWID_BAL_001", WalletType.Personal, "EUR");
	var targetWallet = SeedWallet("AWID_BAL_001", WalletType.Business, "EUR");
	postHandler.Handle(new PostTransactionRequest
	{
		IdempotencyKey = "balance-post-1",
		Awid = "AWID_BAL_001",
		Currency = "EUR",
		Reference = "BALANCE-POST-001",
		CorrelationId = "bal-corr-001",
		PostedBy = "tester",
		Session = "bal-session-001",
		Device = "bal-device-001",
		Lines =
		[
			new LedgerLineRequest { WalletId = sourceWallet.Id, EntryType = EntryType.Debit, Amount = 40m, Description = "balance debit" },
			new LedgerLineRequest { WalletId = targetWallet.Id, EntryType = EntryType.Credit, Amount = 40m, Description = "balance credit" }
		]
	});

	var sourceProjection = balanceService.RebuildFromLedger(sourceWallet.Id);
	var targetProjection = balanceService.RebuildFromLedger(targetWallet.Id);
	Assert(sourceProjection.AvailableBalance == -40m, "debit wallet projection should be -40");
	Assert(targetProjection.AvailableBalance == 40m, "credit wallet projection should be +40");
	Assert(sourceProjection.PendingBalance == 0m && sourceProjection.ReservedBalance == 0m, "projection-only engine keeps pending/reserved as projection outputs");
});

Run("snapshot is rebuild optimization", () =>
{
	var wallet = walletRepository.ListByAwid("AWID_BAL_001").First(candidate => candidate.WalletType == WalletType.Business);
	var first = balanceService.RebuildFromLedger(wallet.Id);
	var second = balanceService.RebuildFromLedger(wallet.Id);
	Assert(first.AvailableBalance == second.AvailableBalance, "rebuild should be deterministic from ledger");
	Assert(second.LastLedgerPosition >= first.LastLedgerPosition, "ledger position should be monotonic");
	var version = balanceService.GetVersion(wallet.Id);
	Assert(version is not null && version.Version >= 2, "projection version should increment after rebuilds");
});

Run("wallet read model exposes detail timeline and portfolio summary", () =>
{
	var awid = "AWID_WALLET_READMODEL";
	var personalWallet = walletRepository.Create(awid, WalletType.Personal, "EUR");
	var businessWallet = walletRepository.Create(awid, WalletType.Business, "EUR");
	postHandler.Handle(new PostTransactionRequest
	{
		IdempotencyKey = "wallet-readmodel-1",
		Awid = awid,
		Currency = "EUR",
		Reference = "WALLET-READMODEL-001",
		CorrelationId = "corr-wallet-readmodel-1",
		PostedBy = "tester",
		Session = "session-wallet-readmodel-1",
		Device = "device-wallet-readmodel-1",
		Lines =
		[
			new LedgerLineRequest { WalletId = personalWallet.Id, EntryType = EntryType.Debit, Amount = 15m, Description = "wallet detail debit" },
			new LedgerLineRequest { WalletId = businessWallet.Id, EntryType = EntryType.Credit, Amount = 15m, Description = "wallet detail credit" }
		]
	});

	var detail = walletReadModelService.GetDetail(personalWallet.Id);
	Assert(detail.AvailableBalance == -15m, "detail should expose the projected balance");
	Assert(detail.LedgerBalance == -15m, "detail should expose the ledger balance");
	Assert(detail.Timeline.Count > 0, "detail should expose at least one timeline item");
	Assert(detail.LastActivityAt != default, "detail should expose a last activity timestamp");

	var portfolio = walletReadModelService.GetPortfolioSummary(awid);
	Assert(portfolio.WalletCount == 2, "portfolio summary should count both wallets");
	Assert(portfolio.TotalAvailable == 0m, "portfolio summary should aggregate balances while preserving double-entry net zero");
	Assert(portfolio.TotalLedgerBalance == 0m, "portfolio summary should aggregate ledger balances while preserving double-entry net zero");
});

if (failures.Count == 0)
{
	Console.WriteLine("All AFW-DLV-0004.2/0004.3 scenarios passed.");
	return;
}
